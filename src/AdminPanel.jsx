        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {events.map((evt, idx) => (
                <tr key={evt.id || `evt-${idx}`} className={`hover:bg-slate-50/50 transition-colors group ${evt.status === 'Inactivo' ? 'opacity-60' : ''}`}>
          ...
          <table className="w-full text-left text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {courses.map((course, idx) => (
                <tr key={course.id || `crs-${idx}`} className={`hover:bg-slate-50/50 transition-colors group ${course.status === 'Inactivo' ? 'opacity-60' : ''}`}>
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6 h-full flex flex-col">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden flex-1">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {slides.map((s, idx) => (
              <tr key={s.id || `sld-${idx}`} className={s.status === 'Inactivo' ? 'opacity-50' : ''}>
    ...
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-6">
      ...
      <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            {ads.map((ad, idx) => (
              <tr key={ad.id || `ad-${idx}`} className={ad.status === 'Inactivo' ? 'opacity-50' : ''}>
                <td className="px-6 py-4">
                  {ad.imageUrl ? (
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              ...
              <select name="position" defaultValue={editingItem?.position || 'Cintillo Superior'} className="w-full border p-2 rounded">
                <option>Cintillo Superior</option><option>Banner Nuevo Superior</option><option>Banner Mitad de Página</option><option>Pop-up</option>
              </select>
            </div>
