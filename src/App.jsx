...
function HomePage() {
  ...
  const [activeSuperiorSlide, setActiveSuperiorSlide] = useState(0);
  ...
  const superiorAds = adsBD.filter(ad => ad.position === 'Banner Nuevo Superior');
  ...
  useEffect(() => {
    if (superiorAds.length <= 1) return;
    const timer = setInterval(() => {
      setActiveSuperiorSlide((prev) => (prev === superiorAds.length - 1 ? 0 : prev + 1));
    }, 5000);
    return () => clearInterval(timer);
  }, [superiorAds.length]);
  ...
  return (
    <div className="min-h-screen bg-[#f4f5f7] font-sans text-slate-800">
      ...
      <div className={currentView === 'home' ? 'block' : 'hidden'}>
        ...
        {/* --- PUBLICIDAD NUEVO BANNER SUPERIOR --- */}
        {superiorAds.length > 0 && (
          <section className="pt-10 md:pt-16 bg-[#f4f5f7]">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="relative w-full overflow-hidden rounded-xl md:rounded-2xl shadow-2xl border border-slate-700 bg-slate-900 group">
                ...
                {superiorAds[0]?.imageUrl ? (
                  <img 
                    src={formatImageUrl(superiorAds[0].imageUrl)} 
                    alt="placeholder" 
                ...
                {superiorAds.map((ad, idx) => (
                  <a key={ad.id || idx} href={ad.link || '#'} target="_blank" rel="noreferrer"
                     className={`absolute inset-0 flex items-center justify-center transition-opacity duration-1000 ease-in-out ${idx === activeSuperiorSlide ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none'}`}
                ...
                {superiorAds.length > 1 && (
                   <div className="absolute bottom-4 right-6 flex gap-2 z-20 bg-black/30 px-3 py-1.5 rounded-full backdrop-blur-md">
                     {superiorAds.map((_, idx) => (
                       <button key={idx} onClick={() => setActiveSuperiorSlide(idx)}
                         className={`h-1.5 md:h-2 rounded-full transition-all duration-500 ${activeSuperiorSlide === idx ? 'w-6 md:w-8 bg-white' : 'w-2 md:w-3 bg-white/40 hover:bg-white/80'}`} 
                       />
                     ))}
                   </div>
                )}
